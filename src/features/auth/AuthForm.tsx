...
import { 
  ActionsWrapper, AuthSubtitle, AuthTitle, ForgotPasswordLink, 
  FormCard, LoginButton, LoginInput, StyledForm, StyledTab, 
  StyledTabs, VisibilityIconButton 
} from "./ui/AuthStyles";
import { LOGIN_QUERY, SIGNUP_MUTATION, LoginResponse, SignupResponse } from "./api/auth.api";
...
export const AuthForm = ({ mode }: { mode: 'login' | 'signup' }) => {
  ...
  const { register, handleSubmit, formState: { errors }, setError } = useForm<AuthInputs>();
  ...
  const [executeLogin, { loading: loginLoading }] = useLazyQuery<LoginResponse>(LOGIN_QUERY);

  const [executeSignup, { loading: signupLoading }] = useMutation<SignupResponse>(SIGNUP_MUTATION, {
    onCompleted: (data) => handleAuthSuccess(data.signup.access_token, data.signup.refresh_token),
    onError: (error) => handleAuthError(error),
  });

  const onSubmit: SubmitHandler<AuthInputs> = async (data) => {
    const variables = { variables: { auth: { email: data.email, password: data.password } } };

    if (mode === 'login') {
      try {
        const result = await executeLogin(variables);
        if (result.data) {
          handleAuthSuccess(result.data.login.access_token, result.data.login.refresh_token);
        }
      } catch (error) {
            ...
            input: {
              endAdornment: (
                <InputAdornment position="end">
                  <VisibilityIconButton onClick={() => setShowPassword(!showPassword)}>
